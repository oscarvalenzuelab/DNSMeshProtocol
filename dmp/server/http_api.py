    ...
    def _handle_root(self) -> int:
        ...
        if reg_self_service and dns_update_enabled:
            ...
            registration_block = (
                ...
                "<p><small>Set a passphrase before <code>dnsmesh init</code>. "
                "It derives your Ed25519 + X25519 keys via Argon2id and is "
                "the only thing protecting your identity — losing it loses "
                "the identity, no recovery. The CLI reads "
                "<code>$DMP_PASSPHRASE</code> first, then the file at "
                "<code>passphrase_file</code> in your config, then prompts "
                "interactively.</small></p>"
                f"<pre>pipx install dnsmesh\n"
                f"read -rs DMP_PASSPHRASE       # silent prompt, not in shell history\n"
                f"export DMP_PASSPHRASE\n"
