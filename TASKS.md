...
- **M1.3** — `ResolverPool.discover()` + `WELL_KNOWN_RESOLVERS` + `dnsmesh resolvers discover [--save]` + `dnsmesh resolvers list` CLI — commit `d39cb56`.
...
- **M2.wire-polish** — Decoupled `dnsmesh cluster pin` from cluster-mode activation. New `cluster_enabled: bool` flag (default False, back-compat for existing configs requires explicit `cluster enable`). New `dnsmesh cluster enable` / `dnsmesh cluster disable` commands; enable runs a live manifest fetch before flipping. `dnsmesh cluster fetch` / `status` work regardless of enable state as pre-enable diagnostics — commits `775c22b..3879663`, merged as `012552a`. 11 new tests.
