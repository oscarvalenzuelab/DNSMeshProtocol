"""Fuzz harness for BootstrapRecord.parse_and_verify.

Property: for ANY bytes / string input and any 32-byte signer_spk, the
parser must return either ``None`` or a ``BootstrapRecord``. It must
never raise.
"""
...
from dmp.core.bootstrap import RECORD_PREFIX, BootstrapRecord
...
def test_parse_never_raises_on_random_text(wire: str) -> None:
    result = BootstrapRecord.parse_and_verify(wire, SOME_SIGNER_SPK)
    assert result is None or isinstance(result, BootstrapRecord)
...
def test_parse_never_raises_on_truncated_prefix(trailing: bytes) -> None:
    result = BootstrapRecord.parse_and_verify(
        RECORD_PREFIX + trailing.hex(), SOME_SIGNER_SPK
    )
    assert result is None or isinstance(result, BootstrapRecord)
...
def test_parse_never_raises_on_arbitrary_signer_spk(spk: bytes) -> None:
    result = BootstrapRecord.parse_and_verify(RECORD_PREFIX + "AAAA", spk)
    assert result is None or isinstance(result, BootstrapRecord)
