...
import pytest
import uuid
from dmp.core.chunking import MessageChunker, MessageAssembler, ChunkInfo, ChunkRouter
...
class TestMessageChunker:
    ...
    def test_chunking_with_error_correction(self):
        ...
        message = DMPMessage(payload=b"Test message for ECC")
    ...
    def test_create_message_chunks(self):
        ...
        original = DMPMessage(
            header=DMPHeader(
                message_id=uuid.uuid4().bytes, message_type=MessageType.DATA
            ),
            payload=b"X" * 300,
        )
...
class TestMessageAssembler:
    ...
    def test_basic_assembly(self):
        ...
        for chunk_num, chunk_data in chunks:
            result = assembler.add_chunk(
                original.header.message_id, chunk_num, chunk_data, len(chunks)
            )
    ...
    def test_corrupted_chunk_rejection(self):
        ...
        result = assembler.add_chunk(message_id, 0, bad_checksum + chunk_data, 1)
...
class TestChunkRouter:
    ...
    def test_route_exclusion(self):
        ...
        destinations = router.get_forward_destinations(
            recipient_id, exclude_nodes=["node2.mesh"]
        )
